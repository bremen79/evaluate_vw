...
def save_vw_dataset(X, y, dname, did, ds_dir, cs, tag):
    if cs:
        n_classes = np.shape(y)[1]
    else:
        n_classes = int(y.max() + 1)

    fname = 'ds_{}_{}_{}_{}.vw.gz'.format(dname, tag, did, n_classes)
...
if __name__ == '__main__':
    ...
    for did in dids:
        ...
        if cs:
            ...
            save_vw_dataset(X, cs_labels, dname, did, VW_DS_DIR, True, tag)
        else:
            tag = 'mc'
            print(X, y)
            save_vw_dataset(X, y, dname, did, VW_DS_DIR, False, tag)
